...
\texttt{01} & +1 & Add \\
\texttt{10} & -1 & Subtract \\
\texttt{11} & 0 & Reserved (Dark State) \\
\bottomrule
\end{tabular}
...
The ``Dark State'' (\texttt{11}) reserves 25\% of the encoding space for future extensions such as activation-dependent weights or learned skip patterns.

\subsection{BitSwitch Forward Pass}

The forward pass processes tiles conditionally:

\begin{algorithm}
\caption{BitSwitch Forward Pass}
\begin{algorithmic}[1]
...
\subsection{BitSwitch Kernel Benchmarks}

We implemented the BitSwitch kernel in C++ with ARM NEON vectorization for the Jetson AGX Thor platform.
...
\bottomrule
\end{tabular}
\caption{BitSwitch kernel speedup with tile sparsity (ARM NEON)}
\label{tab:speedup}
\end{table}
...
\begin{itemize}
    ...
    \item Layers: 6 transformer blocks with BitSwitch FFN
...
We trained a 38.2M parameter language model on TinyStories using BitSwitch sparse layers.
...
\midrule
Parameters & 38.2M \\
Architecture & 12-layer transformer with BitSwitch FFN \\
